...
export default class ClinicDeviceList extends Component {
    ...
    openAddDeviceView(device) {
        window.open(window.location.protocol + '//' + window.location.host + "/addDeviceView?id=" + device.id, '_self');
    }
    ...
    getData() {
        let user = getUser();
        axios.get(apiHost + '/api/organization/device/list?organizationId='+user.clinic.id,
                ...
                if (result.status === 200) {
                    for (let i = 0; i < result.data.length; i++) {
                        ...
                        options.push(<tr key={getKey()}>
                            <td>{i + 1}</td>
                            <td className="linkStyle" onClick={this.openAddDeviceView.bind(this, row)}>{row.deviceName}</td>
                            <td>{row.ipAddress}</td>
